# -*- coding: utf-8 -*-
"""
Created on Sat Jun 27 17:56:56 2020
...
datasets=['cpu','hd','gpu','screen','ram']
...
#    labels_map_count=[l.split(':')[1] for l in aa if(l.startswith('label'))]
    ...
    for i,label in enumerate(test_labels):
        r1_dataframe_lst.append([labels_map_count[label],recalls_top1[i],datas.split('_')[0]])
    for i,label in enumerate(test_labels):
        r5_dataframe_lst.append([labels_map_count[label],recalls_top5[i],datas.split('_')[0]])
    for i,label in enumerate(test_labels):
        f1_1_dataframe_lst.append([labels_map_count[label],f1s_top1[i],datas.split('_')[0]])
    for i,label in enumerate(test_labels):
        f1_5_dataframe_lst.append([labels_map_count[label],f1s_top5[i],datas.split('_')[0]])
...
r1_df=pd.DataFrame(r1_dataframe_lst,columns =['number of samples in the training set','recall@1','class'])
r5_df=pd.DataFrame(r5_dataframe_lst,columns =['number of samples in the training set','recall@5','class'])
...
r1_df.replace({'cpu':'Processor','hd':'Disk','gpu':'Graphic card','screen':'Monitor','ram':'RAM'},inplace=True)
r5_df.replace({'cpu':'Processor','hd':'Disk','gpu':'Graphic card','screen':'Monitor','ram':'RAM'},inplace=True)

f1_1_df.replace({'cpu':'Processor','hd':'Disk','gpu':'Graphic card','screen':'Monitor','ram':'RAM'},inplace=True)
f1_5_df.replace({'cpu':'Processor','hd':'Disk','gpu':'Graphic card','screen':'Monitor','ram':'RAM'},inplace=True)

plt.figure(figsize=(10, 6))

sns.scatterplot(x="number of samples in the training set", y="recall@1",\
                hue="class", data=r1_df)#.set_title('Recall@1')

plt.figure(figsize=(10, 6))

sns.scatterplot(x="number of samples in the training set", y="recall@5",\
